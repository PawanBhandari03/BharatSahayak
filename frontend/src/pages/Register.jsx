...
export default function Register() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const API_URL = (import.meta.env.VITE_API_URL || 'https://bharatsahayak.onrender.com').replace(/\/api\/?$/, '');
      const matchRes = await axios.post(`${API_URL}/api/match-schemes`, backendPayload);
      const matchedBackend = matchRes.data;

      // 2. Fetch AI Recommendations from backend
      const aiRes = await axios.post(`${API_URL}/api/ai-recommend`, {
      ...
      try {
        await axios.post(`${API_URL}/api/users/register`, backendPayload);
      } catch (dbError) {
